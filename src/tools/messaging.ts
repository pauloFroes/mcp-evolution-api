    ...
    async ({ number, status }) => {
      try {
        const data = await apiRequest(
          instanceUrl("updateBlockStatus"),
          "POST",
          { number, status },
        );
        return toolResult(data);
      } catch (error) {
        return toolError(`Failed to update block status: ${(error as Error).message}`);
      }
    },
  );
  ...
  server.registerTool(
    ...
    async ({ number, sticker_url, delay }) => {
      try {
        ...
        const data = await apiRequest(
          instanceUrl("sendSticker"),
          "POST",
          body,
        );
        return toolResult(data);
      } catch (error) {
        return toolError(`Failed to send sticker: ${(error as Error).message}`);
      }
    },
  );
