...
export function registerChatTools(server) {
    ...
    server.registerTool("archive_chat", {
        ...
        try {
            const data = await apiRequest(chatUrl("archiveChat"), "POST", {
            ...
            return toolError(`Failed to archive chat: ${error.message}`);
        }
    });
    ...
    server.registerTool("delete_message", {
        ...
        try {
            const data = await apiRequest(chatUrl("deleteMessage"), "DELETE", {
                key: {
    ...
    // --- Fetch profile ---
    server.registerTool("fetch_profile", {
        title: "Fetch Profile",
        description: "Fetch the WhatsApp profile information of a contact (name, status, etc.).",
        inputSchema: {
            number: z
                .string()
        ...
        try {
            const data = await apiRequest(chatUrl("fetchProfile"), "POST", { number });
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to fetch profile: ${error.message}`);
        }
    });
    // --- Fetch profile picture ---
    server.registerTool("fetch_profile_picture", {
        ...
        try {
            const data = await apiRequest(chatUrl("fetchProfilePictureUrl"), "POST", { number });
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to fetch profile picture: ${error.message}`);
        }
    });
    // --- Find chats ---
    server.registerTool("find_chats", {
        title: "Find Chats",
        description: "List all WhatsApp chats/conversations. Returns chat metadata including last message, unread count, etc.",
        inputSchema: {},
        annotations: {
            readOnlyHint: true,
            destructiveHint: false,
        ...
        try {
            const data = await apiRequest(chatUrl("findChats"), "POST", {});
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to find chats: ${error.message}`);
        }
    });
    // --- Find contacts ---
    ...
    server.registerTool("find_messages", {
        ...
        try {
            const data = await apiRequest(chatUrl("findMessages"), "POST", {
                where: {
                    key: {
                        remoteJid: remote_jid,
                    },
    ...
    server.registerTool("get_base64_from_media", {
        title: "Get Base64 from Media Message",
        description: "Extract media content as base64 from a WhatsApp message. Use this instead of temporary media URLs which expire quickly. Pass the message key ID from find_messages results.",
        inputSchema: {
            message_id: z
                .string()
        ...
        try {
            ...
            if (convert_to_mp4 !== undefined)
                body.convertToMp4 = convert_to_mp4;
            const data = await apiRequest(chatUrl("getBase64FromMediaMessage"), "POST", body);
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to get base64 from media: ${error.message}`);
        }
    });
    // --- Mark as read ---
    server.registerTool("mark_as_read", {
        ...
        try {
            const data = await apiRequest(chatUrl("markMessageAsRead"), "POST", {
    ...
    // --- Send presence ---
    server.registerTool("send_presence", {
        title: "Send Presence",
        description: "Send a presence/typing indicator to a WhatsApp chat (composing, recording, paused, etc.).",
        inputSchema: {
            number: z
                .string()
        ...
        try {
            const data = await apiRequest(chatUrl("sendPresence"), "POST", { number, presence });
            return toolResult(data);
        }
        catch (error) {
            return toolError(`Failed to send presence: ${error.message}`);
        }
    });
    // --- Update message ---
    server.registerTool("update_message", {
        title: "Update Message",
        description: "Edit a previously sent WhatsApp message. Only works for your own messages (fromMe: true).",
        inputSchema: {
            remote_jid: z
                .string()
                .describe("Chat JID (e.g. 5538999999999@s.whatsapp.net or group JID)"),
        ...
        try {
            const number = remote_jid.includes("@")
                ? remote_jid.split("@")[0]
                : remote_jid;
            const data = await apiRequest(chatUrl("updateMessage"), "POST", {
                number,
                text,
                key: {
                    remoteJid: remote_jid,
            ...
            return toolError(`Failed to update message: ${error.message}`);
        }
    });
}
